...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, device, lr, chart_name):
    ...
    for epoch in range(num_epochs):
        ...
        val_accuracy = evaluate_model(mymodel, validation_dataloader, device)['accuracy']
        #val_accs.append(val_accuracy)
        print(f" - Average validation metrics: accuracy={val_accuracy}")

    #plt.plot([i+1 for i in range(num_epochs)], train_accs, label='Train')
    #plt.plot([i+1 for i in range(num_epochs)], val_accs, label='Dev')
    #plt.xlabel('Epochs')
    #plt.ylabel('Accuracy')
...
if __name__ == "__main__":
    ...
    train(pretrained_model, args.num_epochs, train_dataloader, validation_dataloader, args.device, args.lr, args.plot)
    ...
    plt.bar(['dev', 'test'], [val_accuracy, test_accuracy])
    plt.title('Model Accuracies for ' + args.model)
